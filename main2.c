...
void clrscr() {
    system("cls || clear"); // Untuk clear screen setelah user selesai melakukan perhitungan
}
...
int main() {
    ...
    do {
        // Jika pilihan sebelumnya valid, gunakan pilihan itu
        if (pilihan_sebelumnya != -1) {
            pilihan = pilihan_sebelumnya; // Gunakan pilihan sebelumnya
        } else {
        ...
        if (pilihan != 0) {
            // Tanyakan apakah ingin mengulang atau kembali
            char choice;
            printf("Apakah Anda ingin (U)lang atau (K)embali ke menu? ");
            while (getchar() != '\n'); // Kosongkan buffer input
            scanf("%c", &choice);
            if (choice == 'U' || choice == 'u') {
                pilihan_sebelumnya = pilihan; // Simpan pilihan untuk diulang
            } else if (choice == 'K' || choice == 'k') {
                pilihan_sebelumnya = -1; // Reset pilihan sebelumnya
                clrscr(); // Clear screen
            } else {
    ...
    } while (pilihan != 0); // Teruskan hingga pengguna memilih 0
...
void skalaPeta() {
    float jarak_sebenarnya, jarak_peta, skala;

    printf("Masukkan jarak sebenarnya (dalam km): ");
    jarak_sebenarnya = inputAngka();
    printf("Masukkan jarak di peta (dalam cm): ");
    jarak_peta = inputAngka();

    skala = jarak_sebenarnya / (jarak_peta / 100000); // Menghitung skala
    printf("Skala peta: 1 cm = %.2f km\n", skala);
}
